...
8. **Decompose the god-files behind each P1/P2 slice** — carve `main.ts` / `main/index.ts` into `app/` / `state/` / `components/` / `features/` (TODO #6).
   - **Progress — main.ts 4022 → ~2480 (≈38%):** extracted (each a verified pure move) — the **Deck flow** → `features/deck/{config,setup,pickers,media}.ts` (all <400) + a barrel `index.ts`; the **auth flow** (welcome/sync-key/2FA) → `features/auth.ts`; the **hash router** → `lib/router.ts` (a route registry so features register handlers and nothing imports the router cyclically); plus shared `lib/session.ts` (device context), `lib/logos.ts` (logoMap). `StatusMessage` + `DiskPicker` components built and adopted (three hand-built disk renderers → one).
   - **What's left = the ROG core.** The remaining ~2480 lines are the device-pick → customise → apps → ssh → bundles → method → provisioning → profiles → USB-writer → watch flow. They all read/write **~19 shared module-level state vars** (`usbState`, `selectedBaseId`, enabled/disabled modules, selected apps/removals/keys, the `rog*` family, …) and funnel through `gatherUsbRequest`. This is **tightly coupled**, not a self-contained block — so the clean next step is: (1) move the 19 vars into a shared `lib/rog-state.ts` (a large, cross-cutting migration), then (2) split the screens into `features/rog/*`, extracting `gatherUsbRequest` with them. Bigger + riskier than the extractions done so far → do it as a focused, validated pass. `features/auth.ts` (492) also still wants `auth/{welcome,synckey,twofa}.ts`. All oversized files carry recorded-reason headers.
