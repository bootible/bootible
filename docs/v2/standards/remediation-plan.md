...
7. **✅ DONE — Finish the IPC contract.** `StaticIp` de-duplicated (`5d9c72b`, NetworkSettings type-imports from core); `BootibleApi` single-sourced to ambient `lib/bootible-api.d.ts` (P3); and the **68 IPC channel names single-sourced as `CHANNELS` in `app-ipc.ts`** (`7ce9afa`) — preload/main/cloud all reference `CHANNELS.<key>`, migrated via a provably behaviour-preserving exact-string→const substitution (0 raw literals remain, all typecheck, a test pins the 68 unique channels; `node:*` import specifiers untouched). Wants an app-run smoke test. (Coding #5.)
...
12. **PARTIAL — Cross-device contract tests.** ✅ A `cross-device-contract.test.ts` suite (`d19750f`) asserts the family-agnostic guarantees in one place: every capability-registered device has a known family + ≥1 media mode; the Windows family resolves a build choice to modules+settings; the SteamOS family generates a provision script + non-empty bundle; static-IP validation is shared/device-agnostic. REMAINING: wiring the generated-script `bash -n` + PS 5.1 parse into CI (a CI-config change). (coding #16/#17.)
