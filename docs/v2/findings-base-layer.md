...
- **So `asus-drivers` staging (Phase B5) is essentially unnecessary.** "All hardware works" is met by Windows Update out of the box. B5 reduces to at most *ensuring a WU driver scan has run* (automatic with network) — not staging a driver set.
- **The one driver bootible must bake in is Wi-Fi — and it's correct.** `Get-NetAdapter` shows the RC73XA's Wi-Fi is the **MediaTek Wi-Fi 6E MT7922 (RZ616)** — the same chip as the RC72LA, and exactly what bootible already stages. It's *not* inbox in the ISO, so it's required at install/first-logon to give the network that lets Windows Update fetch everything else. One staged driver (MT7922) covers both Ally X generations; nothing else needs baking in. 📄 Observed on-device (driver 3.5.0.1392, MediaTek).
