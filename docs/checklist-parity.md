...
| 5 | Disable CPU boost for battery | Covered | `disable_cpu_boost_on_battery` | `modules/power.ps1` sets `PERFBOOSTMODE 0` on the DC (battery) scheme only; plugged-in performance is untouched. Opt-in (default `false`). |
...
| 13 | Update GPU drivers + guard Windows Update regressions | Not covered | — | `modules/base.ps1` only reports pending Windows updates informationally; no driver updates, no update pause/guard. Re-running `bootible` re-applies settings an update reverted, but that is repair, not prevention. See Deferred. |
...
**Summary: 8 Covered, 2 Partial, 4 Not covered** (of which TDP profiles is a deliberate non-goal, not a gap).
...
- **GPU driver updates + Windows Update guard** — would take an update-policy module (pause/defer Windows Update, detect AMD driver regressions against a known-good version) plus state snapshots to detect what an update changed; this is the planned drift-guard work.
